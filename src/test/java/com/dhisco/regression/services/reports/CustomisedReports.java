package com.dhisco.regression.services.reports;
 ...
 * @since 28-03-2019
 */
@Log4j2 public class CustomisedReports implements IReporter {
	...
	private Function<ITestResult, String> testResultToResultRow(String testName, String suiteName) {
		return testResult -> {
			...
			case ITestResult.SKIP:
				return String
						.format(ROW_TEMPLATE, "warning", suiteName, testName, testResult.getName(), "SKIPPED", "NA");
	...
	private String initReportTemplate() {
		...
		try {
			...
			log.error("Problem initializing template", e);
		}
		return template;
	}
	...
	private void saveReportTemplate(String outputDirectory, String reportTemplate) {
		...
		try {
			...
			log.error("Problem saving template", e);
		}
	}
}
